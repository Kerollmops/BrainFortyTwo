...
/*   Updated: 2015/04/18 15:22:32 by crenault         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Memory::Memory(size_t memSize)
	: _memSize(memSize),
	_offset(0),
	_mem(new unsigned char[this->_memSize]) {
...
Memory					&Memory::operator=(Memory const &rhs) {
	...
	this->_mem = new unsigned char[this->_memSize];
	for (size_t i = 0; i < this->_memSize; ++i) {
		this->_mem[i] = 0;
	}
...
unsigned char const		&Memory::operator[](size_t pos) const {

	return this->_mem[pos];
}

unsigned char const		&Memory::data(void) const {
...
unsigned char			&Memory::data(void) {
